...
static void __app_button_function_cb(char *name, TDL_BUTTON_TOUCH_EVENT_E event, void *argc)
{
    ...
    case TDL_BUTTON_PRESS_DOUBLE_CLICK: {
        if (!sg_chat_bot.is_enable) {
            PR_DEBUG("chat bot not enabled, ignore double click");
            break;
        }

        // Send test text message "你好啊"
        // const char *test_text = "你好啊";
        // OPERATE_RET rt = ai_audio_send_text_message(test_text, strlen(test_text));
        // if (rt == OPRT_OK) {
        //     PR_DEBUG("text message sent successfully: %s", test_text);
        // } else {
        //     PR_ERR("failed to send text message, rt:%d", rt);
        // }
    } break;
    default:
        break;
